...
@property (nonatomic,assign)int messageTime;
@property (nonatomic,strong)SCLButton *selectedBtn;
...
- (SCLTextView *)addTextField:(NSString *)title
{
    ...
    if ([title isEqualToString:@"机主姓名"]) {
        txt.tag = 500;
    }
    
    if([title isEqualToString:@"机主身份证号"]) {
        txt.tag = 501;
        txt.keyboardType = UIKeyboardTypeNumberPad;

    }
    
    if ([title isEqualToString:@"主卡号："]) {
        txt.tag = 600;
        txt.keyboardType = UIKeyboardTypeNumberPad;
    }
...
- (void)keyboardWillShow:(NSNotification *)notification
{
    ...
    [UIView animateWithDuration:0.2f animations:^{
        ...
        f.origin.y -= KEYBOARD_HEIGHT + PREDICTION_BAR_HEIGHT + gap;
        self.view.frame = f;
    }];
...
- (void)hideView
{
    ...
    if ([self.scldelegate respondsToSelector:@selector(SCLNextDone)]) {
        [self.scldelegate SCLNextDone];
    }
}
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    SCLTextView *txetView = (SCLTextView*)textField;
    if (txetView.tag == 501) {
        NSLog(@"ChangeChar  %@",textField.text);
        if ([self.scldelegate respondsToSelector:@selector(SCLSendIDMessage:)]) {
            [self.scldelegate SCLSendIDMessage:[textField.text stringByAppendingString:string]];
        }
    }
    ...
    if(txetView.tag == 600) {
        ...
        if ([self.scldelegate respondsToSelector:@selector(SCLSendNameMessage:)]) {
            [self.scldelegate SCLSendNameMessage:[textField.text stringByAppendingString:string]];
        }
    }
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    ...
    if (txetView.tag == 500) {
        if ([self.scldelegate respondsToSelector:@selector(SCLSendNameMessage:)]) {
            [self.scldelegate SCLSendNameMessage:textField.text];
        }
    }
...
- (void)updateTimer {
    _messageTime --;
    if (_messageTime == 0) {
        CGRect size = _selectedBtn.frame;
        [_selectedBtn setTitle:@"获取短息验证码" forState:UIControlStateNormal];
        _selectedBtn.frame = size;
        [_timer invalidate];
        _timer = nil;
        _selectedBtn.enabled = YES;
        _messageTime = 60;
    }else {
    NSString *time = [NSString stringWithFormat:@"%ds",_messageTime];
     [_selectedBtn setTitle:time forState:UIControlStateNormal];
    }
}
