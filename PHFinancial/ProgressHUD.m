...
//    }
    ...
    if (label.text != nil) {
        ...
        lableRect = [label.text boundingRectWithSize:CGSizeMake(200, 300) options:options attributes:attributes context:NULL];
        lableRect.origin.x = 12;
        lableRect.origin.y = 66;
        
        hudWidth = lableRect.size.width + 24;
        hudHeight = lableRect.size.height + 80;
        
        if (hudWidth < 100) {
            
            hudWidth = 100;
            lableRect.origin.x = 0;
            lableRect.size.width = 100;
...
- (CGFloat)keyboardHeight1 {
    for (UIWindow *testWindow in [[UIApplication sharedApplication] windows]) {
        if ([[testWindow class] isEqual:[UIWindow class]] == NO) {
            for (UIView *possibleKeyboard in [testWindow subviews]) {
                if ([possibleKeyboard isKindOfClass:(@"UIPeripheralHostView")]||[possibleKeyboard isKindOfClass:NSClassFromString(@"UIKeyboard")]){
                    return possibleKeyboard.bounds.size.height;
                }
            }
        }
